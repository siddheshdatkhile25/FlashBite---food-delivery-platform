...
class PaymentServiceDatabaseIntegrationTest {

    @Container
    @SuppressWarnings("resource")
    static final PostgreSQLContainer<?> POSTGRES = new PostgreSQLContainer<>("postgres:16-alpine")
            .withDatabaseName("flashbite_payment")
    ...
    @Test
    void flywayCreatesPaymentTables() {
        Integer historyEntries = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM flyway_schema_history WHERE success = TRUE",
                Integer.class
        );
        Integer paymentsVersionExists = jdbcTemplate.queryForObject(
                """
                SELECT COUNT(*)
        ...
        assertThat(paymentsVersionExists).isEqualTo(1);
    }
}
